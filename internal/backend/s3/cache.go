...
func CacheEntryPath(be *BackendS3, key string) (string, bool) {
	...
	cacheFilePath := filepath.Join(cacheDir, be.Backend.Config.Bucket, be.Backend.Config.Prefix, be.Backend.Config.Key, key)
...
// be false.
func CacheReader(be *BackendS3, key string) (*CacheEntry, bool) {
	if !isCacheEnabled() {
		return nil, false
	}
...
func CacheWriter(be *BackendS3, key string, data []byte) error {
	...
	cacheFileDir := filepath.Join(cacheDir, be.Backend.Config.Bucket, be.Backend.Config.Prefix, be.Backend.Config.Key)

	// Make sure the entire cache directory tree exists
	if err := os.MkdirAll(cacheFileDir, 0o755); err != nil { //nolint:mnd
		return fmt.Errorf("failed to create cache directory: %w", err)
	}

	cacheKey := encodeKey(key)

	cacheFilePath := filepath.Join(cacheFileDir, cacheKey)
