...
func (be *BackendRemote) StateVersions() ([]*tfe.StateVersion, error) {
	...
	// Short-circuit this if we're in sq and no sv is passed. This is the most
	// common sq use case and there's no need to paginate through all the
	// StateVersion records when we know we're always going to take the first one.
	// This makes a noticeable performance difference on slow servers or
	// workspaces with large SV lists.
	sv := be.Cmd.String("sv")
	limit := be.Cmd.Int("limit")
	if (be.Cmd.Name == "sq" || be.Cmd.Name == "si") && sv == "0" {
		limit = 1
	}
