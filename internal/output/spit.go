...
func SliceDiceSpit(raw bytes.Buffer,
	...
	// THINK This is inefficent.  We're forcing a time transformation to occur for
	// all attributes, even though many will not be a timestamp.  One alternative
	// would be to look at first row of full dataset and only add the time
	// transformation to attrs that look like timestamps.
	if cmd.Bool("local") {
		for a := range attrs {
